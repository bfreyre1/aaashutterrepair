...
The quote form `POST`s to `/api/quote`. Required fields: name, phone, email, **street address**, city, job type, description. The route find-or-creates a Kickserv customer (website source `641638`) on account `c56cad` (street on `service_address` plus job notes), then creates a SERVICE ESTIMATE job (`job_type_id` `745511`, `estimate=true`). After Kickserv succeeds, the API fire-and-forgets a wake to the Quo MAIN soft-intake webhook so the customer is texted for the next missing field (usually a day/DATE + time window). Soft-intake wake failures are logged and do **not** fail the quote. Ads conversion events fire only after Kickserv succeeds. If Kickserv fails, the form shows an error (with optional mailto backup) and does **not** claim success. Copy on the page says follow-up is by **text** to schedule.
...
3. Environment variables (Production):
   ...
   - `SOFT_INTAKE_WEBHOOK_URL` (server-only; Quo MAIN soft-intake Cursor webhook. **Brandon must set this on Vercel Production.**)
   - `SOFT_INTAKE_WEBHOOK_AUTH` (server-only; full `Authorization` header value, e.g. `Bearer …`. **Brandon must set this on Vercel Production. Never commit the real token.**)
